...
\begin{document}
	...
	\begin{abstract}
		...
		Experiments on RadioML2016.10A and RadioML2016.10B show that IQFormerLite remains competitive with the state-of-the-art IQFormer while compressing parameters by 65.7\% (0.12~M versus 0.35~M).
			On-device profiling on the RK3588 NPU demonstrates superior efficiency: under INT8 quantization, IQFormerLite achieves a throughput of 6387.84~samples/s and a latency of 0.157~ms, representing a $14.83\times$ speedup over the baseline.
			Furthermore, the framework exhibits high quantization robustness, with only a 0.07 percentage-point accuracy dip during INT8 conversion.
	...
	\begin{figure}[t]
		\centering
		\makebox[\textwidth][c]{\includegraphics[width=0.9\textwidth]{Figures/param_vs_acc_bubble.png}}
	...
	Unless otherwise stated, all reported recognition and interpretability results are aggregated over five independent runs with different random seeds and are presented as mean $\pm$ standard deviation.
	...
	\begin{table}[htbp]
		\centering
		\caption{Comparison of model complexity and accuracy on two datasets (A: RadioML2016.10A, B: RadioML2016.10B). Accuracy values are reported as mean $\pm$ standard deviation. \textbf{Bold} indicates the best result, and \underline{underline} indicates the second-best result.}
	...
	Fig.~\ref{fig:sota_acc} complements Table~\ref{tab:sota_compare} by reporting the mean accuracy as a function of SNR.
	...
	\begin{figure}[htbp]
		\centering
		\subfloat[\centering]{\includegraphics[width=0.49\linewidth]{Figures/RML201610a_sota_acc.pdf}\label{fig:sota_acc_a}}
		\hfill
		\subfloat[\centering]{\includegraphics[width=0.49\linewidth]{Figures/RML201610b_sota_acc.pdf}\label{fig:sota_acc_b}}
		\caption{Comparison of recognition accuracy under varying SNR levels. Curves report the mean; shaded regions denote $\pm$ one standard deviation for IQFormer and IQFormerLite. (a) RadioML2016.10A. (b) RadioML2016.10B. \label{fig:sota_acc}}
	\end{figure}
	...
	Fig.~\ref{fig:mod_acc_comparison}(a)--(d) reports the mean per-class accuracies of IQFormer and IQFormerLite on RadioML2016.10A and RadioML2016.10B.
	...
	\begin{figure}[htbp]
		\centering
		\subfloat[\centering]{\includegraphics[width=0.49\linewidth]{Figures/RML201610a_IQFormer_mod_acc.pdf}\label{fig:mod_acc_a}}
		\hfill
		\subfloat[\centering]{\includegraphics[width=0.49\linewidth]{Figures/RML201610a_IQFormerLite_mod_acc.pdf}\label{fig:mod_acc_b}}
		
		\vspace{6pt}
		
		\subfloat[\centering]{\includegraphics[width=0.49\linewidth]{Figures/RML201610b_IQFormer_mod_acc.pdf}\label{fig:mod_acc_c}}
		\hfill
		\subfloat[\centering]{\includegraphics[width=0.49\linewidth]{Figures/RML201610b_IQFormerLite_mod_acc.pdf}\label{fig:mod_acc_d}}
		
		\caption{Comparison of per-modulation recognition accuracy between the baseline IQFormer and the proposed IQFormerLite. Curves report the mean.
...
%	\end{table}
	...
	\subsection{Spectral Interpretability of the Learnable KAN Filterbank}
	\label{sec:lkf_interpretability}

	To further examine whether the LKF behaves as a structured spectral front-end rather than an opaque parameter block, Fig.~\ref{fig:lkf_interpretability} visualizes the learned filter responses and output spectra of IQFormerLite on RadioML2016.10B.
	The analysis uses the default LKF configuration.
	The learned-filter plots are obtained from the KAN base-branch convolution weights, while the SNR- and modulation-family curves measure the spectra of the LKF output features.
	Therefore, the latter should be interpreted as input-dependent output responses rather than SNR-conditioned changes of the filter kernels.

	\begin{figure}[htbp]
		\centering
		\subfloat[\centering Mean learned LKF response.\label{fig:lkf_interp_a}]{
			\includegraphics[width=0.48\linewidth]{Figures/lkf_A_5seed_mean_learned_filter_response.pdf}
		}
		\hfill
		\subfloat[\centering Cross-seed variability.\label{fig:lkf_interp_b}]{
			\includegraphics[width=0.48\linewidth]{Figures/lkf_B_cross_seed_variability.pdf}
		}
		
		\vspace{0.4em}
		
		\subfloat[\centering Aggregate response envelope.\label{fig:lkf_interp_c}]{
			\includegraphics[width=0.48\linewidth]{Figures/lkf_C_aggregate_response_envelope.pdf}
		}
		\hfill
		\subfloat[\centering LKF output spectra by SNR and modulation family.\label{fig:lkf_interp_d}]{
			\includegraphics[width=0.48\linewidth]{Figures/lkf_D_snr_modulation_family_output_spectra.pdf}
		}
		\caption{Spectral interpretability analysis of the Learnable KAN-based Filterbank (LKF) on RadioML2016.10B.}
		\label{fig:lkf_interpretability}
	\end{figure}

	As shown in Fig.~\ref{fig:lkf_interp_a}, the mean learned-filter response emphasizes near-baseband and low-to-mid frequency components.
	Fig.~\ref{fig:lkf_interp_b} reports the standard deviation across seeds, indicating the cross-seed variability of this frequency-selective pattern.
	The aggregate response envelope in Fig.~\ref{fig:lkf_interp_c} summarizes the overall frequency preference using the mean and $\pm$ one standard deviation.
	Finally, Fig.~\ref{fig:lkf_interp_d} shows that different SNR and modulation-family conditions produce distinct output spectral responses after the fixed learned filterbank.
	...
	Table~\ref{tab:quant_npu} shows that IQFormerLite is consistently more efficient than the baseline IQFormer across all formats. Under FP32, the compiled RKNN size drops from 4261.39~KB to 909.10~KB (78.67\%), while latency decreases from 2.294~ms to 0.197~ms (91.42\%). These reductions translate into an $11.66\times$ throughput increase, from 436.18 to 5084.47~samples/s. The gap is even larger in INT8, where IQFormerLite reaches 6387.84~samples/s, corresponding to a $14.83\times$ increase over the baseline. IQFormerLite also operates at lower NPU load (48\%--49\%) than IQFormer (79\%), suggesting that its operator mix maps more effectively to the RK3588 NPU kernels on this deployment stack.
	
	\begin{table}[htbp]
		\centering
		\caption{On-device performance of IQFormer and IQFormerLite on the Rockchip RK3588 NPU under different quantization types. Quant. \textasciitilde  quantization type; {$\Delta$Mem} \textasciitilde activation memory (runtime RAM overhead); Lat. \textasciitilde per-sample inference latency; Thr. \textasciitilde throughput.}
		\label{tab:quant_npu}
		\renewcommand{\arraystretch}{1.2}
		\begin{adjustbox}{max width=\textwidth}
			...
			\multirow{3}{*}{FP32}
			& IQFormer & 4261.39 & 12.3 & 2.294 & 436.18 & 79 & 61.53 \\
			& \textbf{IQFormerLite} & \textbf{909.10} & \textbf{7.6} & \textbf{0.197} & \textbf{5084.47} & \textbf{49} & \textbf{65.16} \\
			& \textit{Gain (Lite vs. Base)} & {\textit{$\downarrow$78.67\%}} & {\textit{$\downarrow$38.2\%}} & {\textit{$\downarrow$91.42\%}} & {\textit{$\times$11.66}} & {\textit{$\downarrow$38.0\%}} & {\textit{$+3.64$ pp}} \\
			\midrule
			
			\multirow{3}{*}{FP16}
			& IQFormer & 4261.39 & 12.5 & 2.314 & 432.15 & 79 & 61.53 \\
			& \textbf{IQFormerLite} & \textbf{909.10} & \textbf{5.3} & \textbf{0.194} & \textbf{5156.92} & \textbf{48} & \textbf{65.16} \\
			& \textit{Gain (Lite vs. Base)} & {\textit{$\downarrow$78.67\%}} & {\textit{$\downarrow$57.6\%}} & {\textit{$\downarrow$91.62\%}} & {\textit{$\times$11.93}} & {\textit{$\downarrow$39.2\%}} & {\textit{$+3.64$ pp}} \\
			\midrule
			
			\multirow{3}{*}{INT8}
			& IQFormer & 4046.92 & 11.9 & 2.322 & 430.77 & 79 & 53.64 \\
			& \textbf{IQFormerLite} & \textbf{735.38} & \textbf{0.9} & \textbf{0.157} & \textbf{6387.84} & \textbf{48} & \textbf{65.09} \\
			& \textit{Gain (Lite vs. Base)} & {\textit{$\downarrow$81.83\%}} & {\textit{$\downarrow$92.4\%}} & {\textit{$\downarrow$93.25\%}} & {\textit{$\times$14.83}} & {\textit{$\downarrow$39.2\%}} & {\textit{$+11.45$ pp}} \\
			\bottomrule[1.5pt]
			\end{tabular}
		\end{adjustbox}
	\end{table}
	
	IQFormerLite is also stable under post-training quantization on this platform. Moving from FP32 to INT8 reduces accuracy from 65.16\% to 65.09\%, a 0.07 percentage-point change, whereas IQFormer drops from 61.53\% to 53.64\% (7.89 percentage points). INT8 further reduces IQFormerLite's incremental runtime memory to 0.9~MB, compared with 11.9~MB for the baseline under INT8, matching the 92.4\% reduction reported in Table~\ref{tab:quant_npu}. Together, these results indicate that IQFormerLite maintains accuracy while improving throughput and lowering memory overhead under low-precision execution.
	...
	On RK3588, IQFormerLite under INT8 reduces the RKNN footprint by 81.83\% relative to IQFormer and achieves 0.157~ms/sample latency and 6387.84~samples/s throughput with only a 0.07 percentage-point accuracy decrease from FP32.
