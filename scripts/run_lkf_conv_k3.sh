...
MAX_JOBS="${MAX_JOBS:-1}"
DRY_RUN="${DRY_RUN:-0}"
CONSOLE_LOG_DIR="${CONSOLE_LOG_DIR:-logs/lkf_conv_k3_console}"
...
run_one() {
    local dataset="$1"
    local seed="$2"
    local comment="IQFormerLite_lkf-conv_k3_seed${seed}"
    local log_file="${CONSOLE_LOG_DIR}/${dataset}_conv_k3_seed${seed}.log"
    local -a cmd=(
        python main.py
        --model IQFormerLite
    ...
    printf 'Running dataset=%s variant=conv_k3 seed=%s\n' "${dataset}" "${seed}"
...
echo "All requested Conv(k=3) jobs finished."
