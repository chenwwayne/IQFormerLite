...
for model in "${MODELS[@]}"; do
    if [ "$model" == "IQFormerLite" ]; then
        for seed in "${SEEDS[@]}"; do
          tag="${model}_kan_k${KERNEL_SIZE}_g${GRID_SIZE}_r${GRID_RANGE}_seed${seed}_dps7_dp3_bandk64"
          python main.py --model "$model" --aux_mode kan --kernel_size "$KERNEL_SIZE" --grid_size "$GRID_SIZE" --grid_range $GRID_RANGE --seed "$seed" --comment "$tag" --report --database_path /home/cww/dataset/HisarMod2019 --database_choose 2019  --batch_size 512 --band_k 64&
