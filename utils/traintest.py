...
def train_epoch(num_epoch, data_loader, model, min_SNR, max_SNR, optimizer, criterion, device, aux_mode='stft'):
    running_loss = 0
    num_total = 0.0
    model.train()
...
def val_epoch(num_epoch, data_loader, model, min_SNR, max_SNR, scheduler, criterion, device, aux_mode='stft'):
    val_loss = 0.0
    num_total = 0.0
    SNR = dict([(key, 0) for key in range(min_SNR, max_SNR + 1, 2)])
    SNR_true = dict([(key, 0) for key in range(min_SNR, max_SNR + 1, 2)])
...
def test_epoch(num_epoch, data_loader, model, device, aux_mode='stft'):
    y_pred = []
    y_true = []
    SNR = []
    model.eval()
